...
def test_setup_opengl_calls(monkeypatch):
    calls = []
    monkeypatch.setattr(gl, 'glViewport', lambda x, y, w, h: calls.append(('viewport', x, y, w, h)))
    monkeypatch.setattr(gl, 'glEnable', lambda flag: calls.append(('enable', flag)))
    monkeypatch.setattr(gl, 'glBlendFunc', lambda sf, df: calls.append(('blendfunc', sf, df)))
